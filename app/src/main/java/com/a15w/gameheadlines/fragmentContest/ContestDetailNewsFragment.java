 ...
 */
public class ContestDetailNewsFragment extends Fragment {
    @BindView(R.id.fragment_contest_detail_video_rv)
    RecyclerView recyclerView;
    private View view;
    private String dataId;
    private MyRvAdapter myRvAdapter;
    private List<ContestDetailBean.DataBean.RecomendVideosBean> RecomendVideosBeans = new ArrayList<>();

    public static ContestDetailNewsFragment newInstance(){
        return new ContestDetailNewsFragment();
    }
    ...
    private void initData() {
        HttpUtils.init().getContestDetailBean(dataId).enqueue(new Callback<ContestDetailBean>() {
            ...
            public void onResponse(Call<ContestDetailBean> call, Response<ContestDetailBean> response) {
                RecomendVideosBeans.addAll(response.body().getData().getRecomendVideos());
    ...
    class MyRvAdapter extends RecyclerView.Adapter<MyRvViewHolder>{
        ...
        public void onBindViewHolder(MyRvViewHolder holder, int position) {
            ...
            holder.titleTv.setText(RecomendVideosBeans.get(position).getTitle());
            Picasso.with(mContext).load(RecomendVideosBeans.get(position).getThumbnail()).into(holder.imageView);
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            String date = sdf.format(new Date(RecomendVideosBeans.get(position).getUpdateTime()*1000L));
            holder.timeTv.setText(date);
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(mContext, NewsDetailsActivity.class);
                    intent.putExtra("dataId","5605061004");
        ...
        public int getItemCount() {
            return RecomendVideosBeans == null ? 0 :RecomendVideosBeans.size();
        }
    }
}
